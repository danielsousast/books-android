...
import com.example.books.constants.DatabaseConstants
...
class BookDatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    override fun onCreate(database: SQLiteDatabase) {
        database.execSQL(CREATE_TABLE_BOOKS)
        inserBooks(database)
    }
    ...
    companion object {
        ...
        private const val CREATE_TABLE_BOOKS = """
            ...
            ${DatabaseConstants.COLUMNS.TITLE} TEXT NOT NULL,
            ${DatabaseConstants.COLUMNS.AUTHOR} TEXT NOT NULL,
            ${DatabaseConstants.COLUMNS.GENRE} TEXT NOT NULL,
            ${DatabaseConstants.COLUMNS.FAVORITE} INTEGER NOT NULL
            );
        """
    }

    private fun inserBooks(db: SQLiteDatabase) {
        val books = getInitialBooks()

        for (book in books) {
            val values = ContentValues(). apply {
                put(DatabaseConstants.COLUMNS.TITLE, book.title)
                put(DatabaseConstants.COLUMNS.AUTHOR, book.author)
                put(DatabaseConstants.COLUMNS.GENRE, book.genre)
                put(DatabaseConstants.COLUMNS.FAVORITE, if (book.favorite) 1 else 0)
            }
